...
if ($sort != 'ASC') { 
	...
	if(count($filter) > '0') {
				$sql -> query ("SELECT n.uniq, n.title, n.description, n.article, a.name, n.submitted, a.email, a.realname, n.catogory, n.arctime, n.hits, a.avartar, n.sticky, n.id, n.page AS page, n.category_id AS category_id FROM ".$cfg['surfix']."news n LEFT JOIN ".$cfg['surfix']."ncauth a ON n.author_id=a.id ".implode(' ',$joins)." WHERE n.category_id = '${c}' AND (n.page = n.id OR n.page IS NULL) ".(count($new2) > '0' ? 'AND ('.implode(' OR ',$new2).')' : '')." ".(count($new) > '0' ? 'AND ('.implode(' AND ',$new).')' : '')." ORDER BY n.submitted DESC");
								}
			else {
				$sql -> query ("SELECT n.uniq, n.title, n.description, n.article, a.name, n.submitted, a.email, a.realname, n.catogory, n.arctime, n.hits, a.avartar, n.sticky, n.id, n.page AS page, n.category_id AS category_id FROM ".$cfg['surfix']."news n LEFT JOIN ".$cfg['surfix']."ncauth a ON n.author_id=a.id WHERE n.category_id = '${c}' AND (n.page = n.id OR n.page IS NULL) ORDER BY n.submitted $sort LIMIT $max");
	}
